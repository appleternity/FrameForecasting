...
from conll09_frame import *
from globalconfig import *
from sentence import *
from StringIO import StringIO

def read_conll(conll_file, syn_type=None, stringio=False):
    #sys.stderr.write("\nReading {} ...\n".format(conll_file))
        ...
        for l in cf:
            ...
            if l == "":
                if elements[0].sent_num != snum:
                    ...
                    if read_constits:
                        sentence.get_all_parts_of_ctree(cparses[next], CLABELDICT_FRAMEID, True)
                    next_ex += 1
...
def analyze_constits_fes(examples):
    ...
    for ex in examples:
        for fe in ex.invertedfes:
            if fe == FEDICT_FRAMEID.getid(EMPTY_LABEL):
                continue
...
def create_target_lu_map():
    ...
    for lu in root.iter('{http://framenet.icsi.berkeley.edu}lu'):
        ...
        LUDICT_FRAMEID.addstr(target_name)
...
def read_fes_lus(frame_file):
    ...
    for frame in root.iter('{http://framenet.icsi.berkeley.edu}frame'):
        ...
        frid = FRAMEDICT_FRAMEID.addstr(framename)
        frcount += 1
    ...
    for fe in root.iter('{http://framenet.icsi.berkeley.edu}FE'):
        ...
        feid = FEDICT_FRAMEID.addstr(fename)
        fes.append(feid)
    ...
    for lu in root.iter('{http://framenet.icsi.berkeley.edu}lexUnit'):
        ...
        luid = LUDICT_FRAMEID.addstr(lu_fields[0])
        LUPOSDICT_FRAMEID.addstr(lu_fields[1])
        lus.append(luid)
    f.close()
...
def read_frame_maps():
    ...
    sys.stderr.write("Max! {} frame-elements for frame: {}\n\n".format(maxfesforframe, FRAMEDICT_FRAMEID.getstr(longestframe)))
...
def read_related_lus():
    ...
    sys.stderr.write("# Max frames for LU: %d in LU (%s)\n"
                     ...
                     % (max_frames,
                        LUDICT_FRAMEID.getstr(longestlu),
                        tot_lus/tot_frames,
                        avg_frames_per_lu,
                        max_lus,
                        FRAMEDICT_FRAMEID.getstr(longestfrm)))
...
def get_wvec_map():
    ...
    wvf.readline()
    wd_vecs = {VOCDICT_FRAMEID.addstr(line.split(' ')[0]) :
...
def read_frame_relations():
    ...
    for reltype in root.iter('{http://framenet.icsi.berkeley.edu}frameRelationType'):
        ...
        for relation in reltype.findall('{http://framenet.icsi.berkeley.edu}frameRelation'):
            sub_frame = FRAMEDICT_FRAMEID.addstr(relation.attrib["subFrameName"])
            super_frame = FRAMEDICT_FRAMEID.addstr(relation.attrib["superFrameName"])
            ...
            for ferelation in relation.findall('{http://framenet.icsi.berkeley.edu}FERelation'):
                sub_fe = FEDICT_FRAMEID.addstr(ferelation.attrib["subFEName"])
                super_fe = FEDICT_FRAMEID.addstr(ferelation.attrib["superFEName"])
                if sub_fe != super_fe:
    ...
    sys.stderr.write("# descendant frames: %d commonest descendant = %s (%d parents)\n"
                     %(len(xpaths), FRAMEDICT_FRAMEID.getstr(commonest_frame_child), max_num_parents))
    sys.stderr.write("# descendant FEs: %d commonest descendant = %s (%d parents)\n\n"
                     %(len(fe_relations), FEDICT_FRAMEID.getstr(commonest_fe_child), max_parent_fes))
    ...
    with codecs.open("ptb.sents", "w", "utf-8") as ptbsf:
        for constitfile in os.listdir(PTB_DATA_DIR):
            ...
            for p in parses:
                ...
                tokpos = p.pos()
                tokens = [VOCDICT_FRAMEID.addstr(tok) for tok,pos in tokpos]
                postags = [POSDICT_FRAMEID.addstr(pos) for tok,pos in tokpos]
                s = Sentence("constit",sentnum=senno,tokens=tokens,postags=postags,)
                s.get_all_parts_of_ctree(p, CLABELDICT_FRAMEID, False)
                sentences.append(s)
                senno += 1
