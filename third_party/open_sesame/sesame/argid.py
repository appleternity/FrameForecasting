...
if options.mode in ["train", "refresh"]:
    ...
    input_file_name = options.raw_input.split("/")[-1]
    out_conll_file = os.path.join(model_dir, input_file_name)
...
def get_constit_loss(fws, bws, goldspans):
    ...
    for j in xrange(sentlen):
        ...
        for i in xrange(istart, j + 1):
            ...
            constit_ij = to_device(constit_ij, "CPU")
...
def print_part_as_conll(golds, pred_targmaps):
    with codecs.open(out_conll_file, "a", "utf-8") as f:
        for gold, pred in zip(golds, pred_targmaps):
            result = gold.get_str(predictedfes=pred)
            f.write(result + "\n")
        f.close()
...
if options.mode in ["train", "refresh"]:
    ...
    for i, instance in enumerate(instances):
        sys.stderr.write("\x1b[2K\r{} / {} [{:.3f}%]".format(i, length, 1.0*i/length*100))
        ...
        if (i+1) % 1000 == 0:
            print_part_as_conll(temp_instances, predictions)
            temp_instances = []
            predictions = []
