...
optpr.add_option("--file", type=str)
...
data = load_result_file(options.file)
outfile_path = options.file[:-5]+"_targetid.json"

previous_time =  datetime.now()
for count, test in enumerate(data, 1):
    print "\nCount: {} / {} [{:.2f}%] {}".format(
            count, 
            len(data), 
            100.0*count/len(data),
            str(datetime.now())
        )
    start_time = datetime.now()

    try:
        instances = load_instances(test["res"])
    ...
    if count % 2 == 0:
        with codecs.open(outfile_path, 'w', encoding="utf-8") as outfile:
